> 문서: 08-Hello-screen-flow.md · 근거: 코드베이스 분석 · 마지막 갱신: 2026-09-18

> 화면 확인 방법 안내: 06번 문서와 동일하게 이미지 캡처 대신 배포 링크(https://pinder-one.vercel.app/)의 GET 응답을 텍스트로 분석했다(방법·시각은 06번 문서 상단 참고). 이 문서에서 추가로 확인한 것은 "진입 조건"과 관련된 부분이다 — `curl` 원본 HTML을 비교한 결과, SC06(내 일정)·SC08(커뮤니티)은 세션 확인이 끝나기 전에는 헤더·푸터만 있고 본문이 비어 있는 반면(코드의 `sessionLoading` 가드와 일치), **SC09(마이페이지)는 로그인 여부와 무관하게 설정 화면 전체가 그대로 내려온다** — 이는 아래 SC09 항목의 서술을 정정하는 근거다. 실제 화면 이미지가 필요하면 06번 문서 맨 끝 "스크린샷 수동 확보 가이드"를 그대로 따르면 된다(이 문서의 화면 ID도 같은 SC01-SC09를 쓴다).
...
- **SC09(마이페이지)** — 헤더의 "로그인/마이페이지" 링크는 로그인 여부에 따라 SC02 또는 SC09로 연결되지만, 이는 헤더 링크의 편의일 뿐 화면 자체의 접근 제한이 아니다. `/my-page` URL로 직접 접속하면 로그인 여부와 무관하게 화면 전체(닫혀 있는 모달 문구 포함)가 그대로 열린다 — `MyPageClient.tsx`에 진입을 막는 분기가 없다는 것을 코드와 배포 링크 GET 응답 양쪽에서 확인했다(06번 문서 SC09 "정정 사항" 참고). 다만 닉네임 저장·아바타 변경·알림 설정·탈퇴 같은 실제 동작은 서버 API가 세션 부재를 이유로 거부한다. 회원 탈퇴가 완료되면 세션이 삭제되고 SC02로 강제 이동한다(근거: src/components/layout/SiteHeader.tsx, src/app/(main)/my-page/MyPageClient.tsx, src/app/api/account/nickname/route.ts 등 각 API의 `getSessionUser()` 체크).
