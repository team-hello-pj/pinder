> 문서: 09-Hello-exception-flows.md · 마지막 갱신: 2026-09-21

> 비고: 이 문서는 04-Hello-features.md와 06-Hello-screen-design.md에서 이미 확인된 예외 처리를 한 곳에 모은 것이며, 신규 발굴이 아니다. "실제 동작 확인 여부"는 두 값 중 하나다 — **코드 확인**: 소스 코드에서 해당 예외 처리 로직을 직접 확인했다는 뜻이다. **근거 없음(기능 부재 확인)**: 그 상황을 처리하는 코드 자체가 없다는 것을 코드 전체 검색으로 확인했다는 뜻이다.
...
| Kakao 구간(경로) 조회 실패(최적 순서 계산 중) | 해당 구간은 (순서 계산 내부에서만) 직선거리 기반 추정치로 대체되어 계산이 이어짐 — 화면에 표시되는 구간 정보 자체에는 적용되지 않음 | 별도 조치 불필요(추후 "경로 검색"으로 실제 값 재조회 가능) | F2, F7 / SC07 | src/app/planner/PlannerClient.tsx (`runOptimalRoute`의 `fetchLeg` 폴백) | 코드 확인 |
| AI 추천 장소의 좌표를 카카오에서 못 찾음(지역이 일치하는 후보가 없거나 검색 결과 자체가 없음, 대체 후보로 최대 3회 재시도 후에도 실패) | "좌표 매칭에 실패했습니다" 모달 — 일부만 실패하면 "해당 장소는 일정에서 제외했어요"와 함께 나머지 장소로 결과 화면이 채워지고, 전부 실패하면 "조건을 바꾸거나 다시 시도해주세요"만 뜨고 결과 화면 자체가 나타나지 않음 | 나머지 장소로 계속 진행하거나, 조건을 바꿔 "다시 추천받기" 재시도 | F38 / SC06 | src/app/(main)/routes/AiGenerateWizard.tsx (`resolvePlaceCoords`, `matchesRegion`, `unmatchedNotice`) | 코드 확인 |
| 방문지 좌표가 없는 구간(플래너 화면 표시) | 가짜 이동시간·거리·교통수단명을 만들지 않고 "방문지 좌표가 없어 경로를 조회할 수 없습니다" 실패 상태로 표시 | 좌표가 있는 장소로 방문지를 교체하거나 다시 검색해서 추가 | F7 / SC07 | src/app/planner/PlannerClient.tsx (`enrichedSegments`) | 코드 확인 |
