> 문서: 01-Hello-project-overview.md · 마지막 갱신: 2026-09-21
...
이 불편 중 "장소 검색·동선 계산·일정 구성"을 한 화면 안에서 처리하는 방향으로 홈 화면을 설계했다. "목적지를 입력하면 최적화된 일정과 이동 동선을 자동으로 생성해드립니다"라는 문구와 "장소 입력 → AI가 최적의 경로 생성 → 세부 정보 추가해 커스텀"이라는 3단계 설명을 전면에 내세운다(근거: src/app/(main)/page.tsx의 STEPS 배열과 hero 문구).
