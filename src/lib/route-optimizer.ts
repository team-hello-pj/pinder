 ...
 * "얼마나 지도 실측값을 그대로 따를지(metricWeight) vs 얼마나 우선순위/날씨
 * 같은 방문지 사정(comfortWeight)을 반영할지"는 Gemini가 변수를 분석해 만든
 * 가중치를 그대로 받아서 쓴다. 가중치를 이 파일에 고정값으로 두지 않는다.
 */

import { isBadWeather, priorityWeight } from './route-engine';
...
export interface RouteWeights {
  ...
  metricWeight: number;
  /** 방문지 우선순위·날씨 같은 변수 기반 사정을 반영하는 비중 (0~1) */
...
/**
 * 우선순위(급한 방문일수록 큼)와 악천후 속 실외 노출(있으면 큼)을 하나의
 * "늦게 방문할수록 나쁜 정도"(0~1)로 합친다. 규칙 자체는 기존
 * route-engine.ts 의 priorityWeight/isBadWeather 를 그대로 재사용한다.
 */
export function computeDelayCost(place: Place): number {
  const priorityPenalty = (2 - priorityWeight(place)) / 2; // high=1, normal=0.5, low=0
  const weatherPenalty = isBadWeather(place) && OUTDOOR_CATEGORIES.includes(place.category) ? 1 : 0;
  return Math.min(1, priorityPenalty * 0.7 + weatherPenalty * 0.3);
}
 ...
 * comfortWeight 가 클수록 급한/날씨에 취약한 방문지가 앞쪽으로 당겨진다.
