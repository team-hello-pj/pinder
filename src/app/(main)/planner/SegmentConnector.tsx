...
const MODE_ICON: Record<TransportMode, string> = { car: '🚗', walk: '🚶', transit: '🚌', bike: '🚲' };
            ...
            {steps.map((st, i) => (
              <div key={i} className={styles.segmentStep}>
                ...
                <div className={styles.segmentStepBody}>
                  <span className={`${styles.segmentStepLabel} ${styles[`segmentChip-${st.mode}`]}`}>
                    {st.arrowLabel}
                  </span>{' '}
                  {formatDuration(st.minutes)} · {st.distanceKm.toFixed(1)}km
                  {st.nodeLabel ? <div className={styles.segmentStepNode}>{st.nodeLabel}</div> : null}
                </div>
              </div>
            ))}
          </div>
