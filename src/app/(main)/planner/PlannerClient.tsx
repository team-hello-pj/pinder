...
export function PlannerClient() {
  ...
  }, [situationVar, situationSub, situationSeverity]);

  const currentVariableKey = useCallback((): string => {
    const v = currentVariableInput();
    return v ? `${v.id}|${v.sub?.id ?? ''}|${v.severity.id}` : 'none';
  ...
  const setCriteria = (c: RouteCriteria) => {
    ...
    if (
      originId != null &&
      routeOptimization &&
      routeOptimization.signature === buildRouteSignature(originId, places, currentVariableKey())
    ) {
      applyOptimizedOrder(c === 'distance' ? routeOptimization.distance : routeOptimization.time, places);
    }
    for (let idx = 0; idx < segments.length; idx++) {
  ...
  // ---- 최적 경로 계산 (출발지 선택 → 좌표 기반 방문 순서 최적화) ----
  ...
  return (
    <div className={shellClass}>
      ...
      >
        <p className={styles.modalDesc}>
          출발지를{' '}
          <strong>{places.find((p) => p.id === originChoiceId)?.name ?? ''}</strong>
          (으)로 설정하시겠습니까?
        </p>
        <div className={styles.modalActions}>
