...
import { fetchRouteLeg, loadKakaoMapsSdk, searchKeyword, type KakaoPlaceDoc } from '@/lib/kakao/client';
...
import { DEV_ROLE_OPTIONS, INITIAL_EDIT_REQUESTS, INITIAL_MEMBERS, INITIAL_PLACES, type DevRole } from './data';
...
const SESSION_LOGIN_FLAG = 'pnder_google_logged_in';
...
export function PlannerClient() {
  ...
  const [segments, setSegments] = useState<TransportMode[]>(isNewRoute ? [] : ['car', 'walk', 'transit']);
  ...
  const [tripStart, setTripStart] = useState(isNewRoute ? new Date().toISOString().slice(0, 10) : '');
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 지도 인스턴스는 마운트 시 한 번만 생성한다
  }, []);
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem(SESSION_LOGIN_FLAG) === '1') setIsLoggedIn(true);
    } catch {
      // 세션 스토리지 접근 실패는 무시 — 로그인 안 된 상태로 계속 진행.
    }
  ...
  const confirmAddPlace = async () => {
    ...
    showToast(geo ? '방문지가 일정에 추가됐어요' : '방문지가 추가됐어요 (좌표를 찾지 못해 지도에는 표시되지 않아요)');
  };
  ...
  const cyclePriority = (id: number) => {
    ...
    logActivity(`${place.name}의 우선순위를 ${next === 'high' ? '급한 방문' : next === 'low' ? '낮음' : '보통'}(으)로 변경했습니다`);
  };
  const changeDuration = (id: number, delta: number) => {
    const place = places.find((p) => p.id === id);
    setPlaces((prev) => prev.map((p) => (p.id === id ? { ...p, duration: Math.max(5, p.duration + delta) } : p)));
    if (place) logActivity(`${place.name}의 체류시간을 ${Math.max(5, place.duration + delta)}분으로 변경했습니다`);
  };
  ...
  const searchAllRoutes = async () => {
    ...
    for (let idx = 0; idx < segments.length; idx++) {
      for (const mode of MODE_ORDER) tasks.push(fetchSegmentRouteForModeWithCriteria(idx, mode, criteria));
    }
  ...
  const applySituation = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const result = applySituationAdjustment(situationVar, situationSub, sevWeight, places, segments);
  ...
  const copyInvite = () => {
    const link =
      permission === 'edit' ? 'pinder.app/invite/8f2xk1?role=editor' : 'pinder.app/invite/8f2xk1?role=viewer';
  ...
  const buildRouteContext = () => ({
    places: places.map((p, i) => ({ order: i + 1, name: p.name, address: p.address, durationMin: p.duration })),
  ...
  const sendAiMessage = async (overrideText?: string) => {
    ...
    try {
      ...
      setAiMessages((prev) => [...prev, { role: 'ai', text: 'AI 응답을 가져오지 못했어요. 잠시 후 다시 시도해주세요.' }]);
    } finally {
      setAiLoading(false);
    }
  };
      ...
      segments.map((mode, idx) => {
        ...
        if (cached && !('failed' in cached)) {
          if (mode === 'transit' && cached.transitSteps?.length) {
            ...
            steps = [{ mode, arrowLabel: mode, minutes: cached.minutes, distanceKm: cached.distanceKm, nodeLabel: null }];
          }
        ...
        const expanded =
          mode === 'transit' && hasMultiStep ? expandedSegments[idx] !== false : Boolean(expandedSegments[idx]);
  ...
  return (
    <div className={shellClass}>
      ...
      <div className={styles.panel}>
        {panelCollapsed ? (
          <button type="button" className={styles.rail} onClick={togglePanelCollapsed} aria-label="플래너 펼치기">
            »
          </button>
        ) : (
          <>
            <div className={styles.panelHeader}>
              <div className={styles.panelHeaderTop}>
                ...
                <div className={styles.panelHeaderActions}>
                  {canManageInvite ? (
                    <button type="button" className={styles.chipBtn} onClick={() => setPermissionsModalOpen(true)}>
                      권한 관리
                    </button>
                  ) : null}
                  {canManageInvite ? (
                    <button type="button" className={styles.chipBtn} onClick={() => setInviteOpen((v) => !v)}>
                      + 일행 초대
                    </button>
                  ) : null}
                  <button type="button" className={styles.collapseBtn} onClick={togglePanelCollapsed} title="플래너 접기">
                    «
                  </button>
                </div>
              </div>
              ...
              <div className={styles.devRoleRow}>
                ...
                {DEV_ROLE_OPTIONS.map((ro) => (
                  <button
                    key={ro.key}
                    type="button"
                    className={devRole === ro.key ? `${styles.devRoleChip} ${styles.devRoleChipActive}` : styles.devRoleChip}
              ...
              {inviteOpen ? (
                <div className={styles.inviteBox}>
                  <p className={styles.inviteDesc}>
                    링크가 있는 사람은 {permission === 'edit' ? '함께 수정할 수 있어요' : '보기만 할 수 있어요'}
                  </p>
            ...
            <div className={styles.criteriaSection}>
              ...
              <div className={styles.segmentedGroup}>
                {(['time', 'distance'] as RouteCriteria[]).map((c) => (
                  <button
                    key={c}
                    type="button"
                    className={criteria === c ? `${styles.segmentedItem} ${styles.segmentedItemActive}` : styles.segmentedItem}
              ...
              <div className={styles.modeChipRow}>
                {MODE_ORDER.map((mode) => (
                  ...
                  >
                    {mode === 'car' ? '🚗' : mode === 'walk' ? '🚶' : mode === 'transit' ? '🚌' : '🚲'}{' '}
                    {mode === 'car' ? '자동차' : mode === 'walk' ? '도보' : mode === 'transit' ? '대중교통' : '자전거'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className={styles.addBar}>
              ...
              {canEdit ? <p className={styles.addHint}>여러 줄 붙여넣기·엑셀 업로드도 지원할 예정</p> : null}
            </div>
          </>
        )}
      </div>
      ...
      <div className={styles.mapCol}>
        {mapCollapsed ? (
          <button type="button" className={styles.rail} onClick={toggleMapCollapsed} aria-label="지도 펼치기">
            «
          </button>
        ) : (
          <>
            <div className={styles.mapArea}>
              ...
              {searchMode ? (
                <div className={styles.searchOverlay}>
                  <div className={styles.searchBox}>
                    ...
                    />
                    <button type="button" className={styles.searchRunBtn} onClick={runMapSearch} aria-label="검색">
                      🔍
                    </button>
                    <button
                      type="button"
                  ...
                  {mapSearchLoading ? <div className={styles.searchStatusBox}>검색 중...</div> : null}
                  {mapSearchError ? (
                    <div className={`${styles.searchStatusBox} ${styles.searchStatusError}`}>{mapSearchError}</div>
                  ) : null}
                  ...
                  {selectedMapDoc ? (
                    <div className={styles.selectedDocCard}>
                      ...
                      <button type="button" className={styles.selectedDocBtn} onClick={confirmMapSelection}>
                        이 장소 선택
                      </button>
                    </div>
                  ) : null}
              ...
              {!kakaoReady ? (
                <div className={styles.mapStatusBadge}>
                  {kakaoLoadFailed ? '지도 로드 실패 · 카드로 계속 진행하세요' : '지도 불러오는 중...'}
                </div>
              ) : null}
              ...
              <div className={aiOpen ? `${styles.aiPanel} ${styles.aiPanelOpen}` : styles.aiPanel}>
                <div className={styles.aiHead}>
                  ...
                  <button type="button" className={styles.aiCloseBtn} onClick={() => setAiOpen(false)} aria-label="닫기">
                    ✕
                  </button>
                </div>
                ...
                <div className={styles.aiMessages}>
                  {aiMessages.map((msg, i) => (
                    <div key={i} className={styles.aiMessageWrap} style={{ alignItems: msg.role === 'ai' ? 'flex-start' : 'flex-end' }}>
                      <div className={msg.role === 'ai' ? styles.aiBubbleAi : styles.aiBubbleUser}>{msg.text}</div>
                      {msg.role === 'ai' && i === aiMessages.length - 1 && msg.suggestions?.length ? (
            ...
            <div className={styles.summaryBar}>
              <div className={styles.summaryStats}>
                ...
                <div>
                  ...
                  <p className={styles.summaryValue}>{places.length ? `${totalDistance.toFixed(1)}km` : '-'}</p>
                </div>
                <div>
              ...
              <div className={styles.summaryActions}>
                {canDeleteOriginal ? (
                  <button type="button" className={styles.deleteOriginalBtn} onClick={deleteOriginalRoute}>
                    원본 삭제
                  </button>
                ) : null}
                {canEdit ? (
                  <button type="button" className={styles.situationBtn} onClick={openSituationModal}>
                    변수 추가
                  </button>
                ) : null}
                <button type="button" className={styles.actionBtn} onClick={() => setActivityLogOpen(true)}>
                  활동 로그
                </button>
                <button type="button" className={styles.actionBtn} onClick={saveOrRemoveAction}>
                  {isViewerRole ? (devRole === 'savedViewer' ? '내 일정에서 제거 ✓' : '내 일정에 저장') : saveLabel}
                </button>
                {devRole === 'viewer' && editRequestStatus === 'none' ? (
                  <button type="button" className={styles.requestEditBtn} onClick={requestEditPermission}>
                    편집 권한 요청
                  </button>
                ) : null}
                ...
                <button type="button" className={styles.myRoutesBtn} onClick={() => setMyRoutesOpen(true)}>
                  내 일정
                </button>
              </div>
            </div>
      ...
      {/* 내 일정 드로어 */}
      {myRoutesOpen ? <div className={styles.drawerOverlay} onClick={() => setMyRoutesOpen(false)} /> : null}
      <div className={styles.myRoutesDrawer} style={{ transform: myRoutesOpen ? 'translateX(0)' : 'translateX(100%)' }}>
        <div className={styles.drawerHead}>
          <span>내 일정</span>
          <button type="button" className={styles.drawerCloseBtn} onClick={() => setMyRoutesOpen(false)}>
            ✕
          </button>
        </div>
        <div className={styles.drawerBody}>
          {savedRoutes.length > 0 ? (
            savedRoutes.map((route) => (
              <div key={route.id} className={styles.savedRouteCard} onClick={() => loadSavedRoute(route.id)}>
                <div className={styles.savedRouteInfo}>
                  <p className={styles.savedRouteName}>{route.title}</p>
                  <p className={styles.savedRouteMeta}>
                    {new Date(route.updatedAt).toLocaleDateString('ko-KR')} · {route.places.length}곳
                  </p>
                </div>
                <button
                  type="button"
      ...
      {/* 활동 로그 드로어 */}
      {activityLogOpen ? <div className={styles.drawerOverlay} onClick={() => setActivityLogOpen(false)} /> : null}
      <div className={styles.activityDrawer} style={{ transform: activityLogOpen ? 'translateX(0)' : 'translateX(100%)' }}>
        <div className={styles.drawerHead}>
          <span>활동 로그</span>
          <button type="button" className={styles.drawerCloseBtn} onClick={() => setActivityLogOpen(false)}>
            ✕
          </button>
        </div>
        <div className={styles.nicknameRow}>
          <span>닉네임</span>
          <input value={nickname} onChange={(e) => setNickname(e.target.value)} className={styles.nicknameInput} />
        </div>
        <div className={styles.drawerBody}>
      ...
      {/* 새 일정 날짜 모달 */}
      <Modal open={newTripDateModalOpen} title="새 일정 만들기" onClose={() => setNewTripDateModalOpen(false)}>
        ...
        <div className={styles.dateFieldGroup}>
          ...
          <label className={styles.dateField}>
            <span>종료 날짜</span>
            <input type="date" value={tripEnd} min={tripStart} onChange={(e) => setTripEnd(e.target.value)} />
          </label>
        </div>
      ...
      {/* 권한 관리 모달 */}
      <Modal open={permissionsModalOpen} title="권한 관리" onClose={() => setPermissionsModalOpen(false)}>
              ...
              {editRequests.map((req) => (
                <div key={req.id} className={styles.requestRow}>
                  ...
                  <div className={styles.requestActions}>
                    <button type="button" className={styles.approveBtn} onClick={() => askApproveRequest(req.id)}>
                      승인
                    </button>
                    <button type="button" className={styles.rejectBtn} onClick={() => askRejectRequest(req.id)}>
                      거절
                    </button>
                  </div>
                </div>
      ...
      <Modal open={requestConfirmOpen} title="확인" onClose={() => setRequestConfirmOpen(false)}>
        ...
        <div className={styles.modalActions}>
          ...
          <Button variant={requestConfirmKind === 'reject' ? 'danger' : 'primary'} size="sm" onClick={confirmRequestAction}>
      ...
      {/* 로그인 필요 안내 */}
      <Modal open={loginRequiredModalOpen} title="로그인이 필요해요" onClose={() => setLoginRequiredModalOpen(false)}>
      ...
      <Modal open={editModalOpen} title="방문지 수정" onClose={() => setEditModalOpen(false)}>
        <div className={styles.field}>
          ...
          <input value={editName} onChange={(e) => setEditName(e.target.value)} className={styles.fieldInput} />
        </div>
        <div className={styles.field}>
          <span className={styles.fieldLabel}>주소</span>
          <input value={editAddress} onChange={(e) => setEditAddress(e.target.value)} className={styles.fieldInput} />
        </div>
        <div className={styles.modalActions}>
      ...
      {/* 카테고리 선택 */}
      <Modal open={categoryModalOpen} title="카테고리 선택" onClose={() => setCategoryModalOpen(false)}>
        <div className={styles.categoryGrid}>
          {CATEGORY_OPTIONS.map((cat) => (
            <button key={cat} type="button" className={styles.categoryOption} onClick={() => selectCategory(cat)}>
              {cat}
            </button>
          ))}
        </div>
      ...
      {/* 방문지 추가 확인 */}
      <Modal open={addConfirmOpen} title="방문지를 추가할까요?" onClose={() => setAddConfirmOpen(false)}>
        <div className={styles.field}>
          <span className={styles.fieldLabel}>이름</span>
          <input value={pendingName} onChange={(e) => setPendingName(e.target.value)} className={styles.fieldInput} />
        </div>
        <div className={styles.field}>
          <span className={styles.fieldLabel}>주소</span>
          <input value={pendingAddress} onChange={(e) => setPendingAddress(e.target.value)} className={styles.fieldInput} />
        </div>
        <div className={styles.modalActions}>
      ...
      {/* 상황 변경 */}
      <Modal open={situationModalOpen} title="변수 추가" onClose={() => setSituationModalOpen(false)}>
        ...
        <div className={styles.situationList}>
          {SITUATION_VARS.map((sv) => (
            <button
              key={sv.id}
              type="button"
              className={situationVar === sv.id ? `${styles.situationOption} ${styles.situationOptionActive}` : styles.situationOption}
            ...
            <div className={styles.situationRow}>
              {WEATHER_SUBS.map((wo) => (
                <button
                  key={wo.id}
                  type="button"
                  className={situationSub === wo.id ? `${styles.situationChip} ${styles.situationOptionActive}` : styles.situationChip}
            ...
            <div className={styles.situationRow}>
              {SEVERITY_LEVELS.map((sev) => (
                <button
                  key={sev.id}
                  type="button"
                  className={situationSeverity === sev.id ? `${styles.situationChip} ${styles.situationOptionActive}` : styles.situationChip}
        ...
        <div className={styles.modalActions} style={{ marginTop: 16 }}>
          ...
          <Button
            ...
            disabled={!situationVar || !situationSeverity || (situationVar === 'weather' && !situationSub)}
          >
            동선 재계산
          </Button>
        </div>
      ...
      <Modal
        ...
        title={deleteTargetId === 'ORIGINAL_ROUTE' ? '정말 삭제하시겠습니까?' : `${deleteTargetLabel}을 삭제하시겠습니까?`}
        ...
        {deleteTargetId === 'ORIGINAL_ROUTE' ? (
          <>
            ...
            <label className={styles.agreeRow}>
              <input type="checkbox" checked={deleteAgreeChecked} onChange={() => setDeleteAgreeChecked((v) => !v)} />
