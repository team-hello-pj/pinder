...
1. **내 일정** — 홈에서 **"최적 동선 생성하기"** 클릭 → 로그인 진행 → 미리 제작된 **내 일정**으로 진입

2. **온보딩 및 일정 편집** — 전체보기에서는 일정 추가가 불가능함을 안내 → **2일차**로 이동 → 지도에서 장소를 클릭해 방문지를 추가하는 과정 시연 → **"경로 계산"**으로 이동

3. **변수 반영** — **"변수 추가"**에서 상황을 입력하고 AI가 동선을 재구성하는 과정 시연 → 예: **"디저트가 많아요"**

4. **AI 일정 생성** — **AI 일정 생성**에서 하루 일정 선택 → 여행 테마 설정 → AI가 방문지와 순서를 생성하는 과정 시연 → 필요하면 **"다시 추천받기"**로 재생성 → 경로 계산 시 출발지가 목록에 없으면 **현위치 선택 → "내 위치"**로 이름 변경 → 일정에서 나간 후 **저장 버튼** 확인

5. **여행지 탐색** — **"여행지 탐색"**에서 경북 선택 → 포항 진입 → AI 챗에서 **"포항에서 진짜 유명한 곳으로 추천해줘"** 입력 → 추천 결과에서 **"호미곶을 일정에 추가해줘"**라고 요청해 일정에 추가 → 제시된 예시 문항을 선택해 추가하는 과정 시연

6. **공유 및 권한** — **"일행 초대"**로 일정 공유 → 다른 팀원이 공유 링크로 접속해 참여 신청 → **권한 관리**를 통해 편집/보기 권한을 구분하는 흐름 설명 → 보기 권한에서는 수정이 불가능함을 보여주고 → 편집 권한 화면에서 **알림 신청** 기능까지 간단히 언급

7. **완료 일정 및 커뮤니티** — 완료된 일정을 간단히 확인 → **커뮤니티**에서 방문 기록 및 후기 기능을 간단히 소개

8. **마무리** — 주요 기능 시연을 마치고 종료
